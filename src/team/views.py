...
def cancel_plan(request):
    ...
    try:
        ...
        stripe.Subscription.delete(team.stipe_subscription_id)
    except Exception:
...
def check_session(request):
    ...
    try:
        team = Team.objects.filter(members__in=[request.user]).first()
        subscription = stripe.Subscription.retrieve(team.stipe_subscription_id)
...
def stripe_webhook(request):
    ...
    try:
        event = stripe.Webhook.construct_event(
            payload, signature_header, webhook_key
        )
    ...
    if event['type'] == 'checkout.session.completed':
        ...
        team.stipe_customer_id = session.get('customer')
        team.stipe_subscription_id = session.get('subscription')
        team.save()
